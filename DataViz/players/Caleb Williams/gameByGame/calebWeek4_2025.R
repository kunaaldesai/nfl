...
# ---- Plot: Play-by-Play EPA (Week 4) ----
ggplot(caleb_pbp_week_4, aes(x = play_index, y = epa)) +
  ...
  geom_hline(yintercept = caleb_game_avg_epa_week_4, linetype = "dotted", alpha = 0.8) +
  geom_hline(yintercept = 0, linetype = "dotdash", alpha = 0.6) +
  # Label biggest positive/negative plays
  ggrepel::geom_text_repel(
    data = label_candidates_week_4,
  ...
  labs(
    title = "C.Williams — Play-by-Play EPA (Week 4)",
    subtitle = paste0("Opponent: ", caleb_opp_week_4, 
                      "   ·   Dotted = Game avg (", round(caleb_game_avg_epa_week_4, 3), 
                      "),  Dot-dash = 0  ·  Rolling line = 5-play mean"),
    x = "Play Sequence (Week 4)",
    y = "EPA",
    shape = "Play Type"
  ) +
