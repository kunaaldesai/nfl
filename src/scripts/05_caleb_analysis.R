...
caleb_pbp_week_3 <- pbp_25 %>%
  filter(week == 3, name == "C.Williams") %>%
  filter(!is.na(epa)) %>%
  arrange(game_id, desc(game_seconds_remaining), qtr, desc(quarter_seconds_remaining)) %>%
  mutate(
    play_index = row_number(),
...
caleb_pbp_week_4 <- pbp_25 %>%
  filter(week == 4, name == "C.Williams") %>%
  filter(!is.na(epa)) %>%
  arrange(game_id, desc(game_seconds_remaining), qtr, desc(quarter_seconds_remaining)) %>%
  mutate(
    play_index = row_number(),
...
caleb_opp_week_3 <- caleb_pbp_week_3 %>% distinct(defteam) %>% pull(defteam) %>% paste(collapse = "/")
caleb_game_avg_epa_week_3 <- mean(caleb_pbp_week_3$epa, na.rm = TRUE)
caleb_opp_week_4 <- caleb_pbp_week_4 %>% distinct(defteam) %>% pull(defteam) %>% paste(collapse = "/")
caleb_game_avg_epa_week_4 <- mean(caleb_pbp_week_4$epa, na.rm = TRUE)


# Label only the most impactful snaps (top-3 & bottom-3 EPA)
label_candidates_week_1 <- bind_rows(
...
# Label only the most impactful snaps (top-3 & bottom-3 EPA)
label_candidates_week_2 <- bind_rows(
  caleb_pbp_week_2 %>% arrange(desc(epa)) %>% slice_head(n = 3),
  caleb_pbp_week_2 %>% arrange(epa)      %>% slice_head(n = 3)
) %>%
  mutate(
    label_text = paste0(
      "Q", ifelse(!is.na(qtr), qtr, "?"),
...
# Label only the most impactful snaps (top-3 & bottom-3 EPA)
label_candidates_week_3 <- bind_rows(
  caleb_pbp_week_3 %>% arrange(desc(epa)) %>% slice_head(n = 3),
  caleb_pbp_week_3 %>% arrange(epa)      %>% slice_head(n = 3)
) %>%
  mutate(
    label_text = paste0(
      "Q", ifelse(!is.na(qtr), qtr, "?"),
...
label_candidates_week_4 <- bind_rows(
  caleb_pbp_week_4 %>% arrange(desc(epa)) %>% slice_head(n = 3),
  caleb_pbp_week_4 %>% arrange(epa)      %>% slice_head(n = 3)
) %>%
  mutate(
    label_text = paste0(
      "Q", ifelse(!is.na(qtr), qtr, "?"),
